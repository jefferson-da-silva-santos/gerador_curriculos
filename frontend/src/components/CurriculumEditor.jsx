...
const inlineStyles = {
 ...
 linkFieldLabel: {
  ...
  // Estilo para o campo de ícone (select) para não quebrar o layout
  iconSelectField: {
      ...
      <div className="editor-form-column">
       ...
       <Form className="editor-form">
        ...
        <div className="input-group-section section-personal">
         ...
         {/* 🌟 SEÇÃO ALTERADA: Links Dinâmicos */}
           ...
           <div className="links-array-section">
            {values.contact.links.map((link, index) => (
             <div key={index} style={inlineStyles.linkItemGrid}>
              <div>
               <label style={inlineStyles.linkFieldLabel}>Ícone (Selecionar):</label>
              ...
              >
                {/* Mapeia o array de ICON_OPTIONS para gerar as opções do select */}
            ...
            <button
             ...
             onClick={() => push({ label: '', url: '', handle: '', icon: ICON_OPTIONS[0].class })}
            >
          ...
          <div className="input-group-section">
           {values.education.map((edu, index) => (
            <div className="formacao form-item-border" key={index}>
             ...
             <button type="button" onClick={() => remove(index)} className="remove-button-margin btn-delete">
          ...
          <div className="input-group-section">
           {values.skills.map((skill, index) => (
            <div key={index} style={inlineStyles.skillItemRow}>
             ...
             <button className="btn-del" type="button" onClick={() => remove(index)} title="Remover Competência">
              <i className="bx bx-trash"></i>
             </button>
            </div>
           ))}
           <button className="btn-add-link" type="button" onClick={() => push({ name: '', level: 3 })}>
             ...
             <FieldArray name={`experience.${expIndex}.responsibilities`}>
              {({ push: pushResp, remove: removeResp }) => (
               <div className="responsas responsa-list-indent">
                ...
                <button className="btn-add-link" type="button" onClick={() => pushResp('')}>
                 + Responsabilidade
                </button>
               </div>
              )}
             </FieldArray>
             <button type="button" onClick={() => remove(expIndex)} className="btn-delete">
